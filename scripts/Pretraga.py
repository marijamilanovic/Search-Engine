...
def standardQuery(inDir, inWord):
    if (len(inWord) > 0):                                                   # dozvoljeno do 3 reci
        ...
        if (len(sub_str) == 2 and (sub_str[0] != sub_str[1])):              # 2 reci
            print("1. rec je " + str(sub_str[0]) + str("."))
            print("2. rec je " + str(sub_str[1]) + str("."))
        ...
        if (len(sub_str) == 3 and (sub_str[0] != sub_str[1] and sub_str[1] != sub_str[2] and sub_str[0] != sub_str[2])):
            print("1. rec je " + str(sub_str[0]) + str("."))
            print("2. rec je " + str(sub_str[1]) + str("."))
            print("3. rec je " + str(sub_str[2]) + str("."))
    ...
    if (len(inWord) > 0):
        ...
        if (len(sub_str) > 1 and (sub_str[1].lower() in logicalOperators) and len(sub_str) == 3):
            ...
            if (sub_str[1].lower() in logicalOperators[0]):         # AND
                print("1. rec je " + str(sub_str[0]) + str("."))
                print("2. rec je " + str(sub_str[2]) + str("."))
                ispars1, curr1 = newDictionary(sub_str[0].lower(), inDir)
            ...
            print("Uneli ste vise od tri reci...")
            return None
...
def searchStandardOR(dict1, dict2, dict3):
    ...
    for key3 in dict3.keys():
        set3.add(key3)
    rezz = set1.union(set2.union(set3))
    for i in dict1.keys():      # presek sva tri
        if i in dict2.keys():
            if i in dict3.keys():
                finito[i] = dict1[i] + dict2[i] + dict3[i]      # saberi njihove vrednosti i stavi u recnik
                #print("kljuc: " + str(i) + " ------ " + str(finito[i]))
    for i in dict1.keys():      # samo u prvom
        if((i not in dict2.keys()) and (i not in dict3.keys())):
                finito[i] = dict1[i]
                #print("kljuc: " + str(i) + " ------ " + str(finito[i]))
    for i in dict2.keys():      # samo u drugom
        if((i not in dict1.keys()) and (i not in dict3.keys())):
                finito[i] = dict2[i]
                #print("kljuc: " + str(i) + " ------ " + str(finito[i]))
